...
SELECT * FROM Payments;
	...
	-- Examples [ INNER LEFT RIGHT FULL CROSS SELF JOIN ] --

-- Retrieve the full name of all customers by orders  [INNER JOIN]
 SELECT c.Customer_Id, o.Order_Id, c.Full_Name FROM Customers c INNER JOIN Orders o ON c.Customer_Id = o.Customer_Id ;
 
 -- Find productid productName price by orders [ LEFT JOIN ]  
 SELECT p.Product_Id, p.Product_Name, p.price FROM Products p LEFT JOIN Orders o ON P.Product_Id = o.Product_Id ;
 
 -- Display the Order ID, the Amount, and the Full Name of the customer associated with it [ RIGHT JOIN ]
 SELECT o.Order_Id, o.Amount, c.Full_Name FROM Customers c RIGHT JOIN Orders o ON c.Customer_Id = o.Customer_Id ;

 -- Retrieve the customers and the Orders  [ FULL JOIN ]
 SELECT c.Customer_Id , o.order_Date FROM Customers c LEFT JOIN Orders o ON c.Customer_id = o.Customer_Id  
 UNION
 SELECT c.Customer_Id , o.Amount  FROM Customers c RIGHT JOIN Orders o ON c.Customer_Id = o.Customer_Id ; -- RIGHT JOIN 
 
 --  Multiple INNER JOINS [Three-Table Join]
 
 -- Find the full name of customers, the product name they ordered, and the payment status for those orders.
 SELECT C.Full_Name, P.Product_Name, Py.Payment_Status FROM Customers C INNER JOIN Orders O ON C.Customer_Id = O.Customer_Id
 INNER JOIN Products P ON O.Product_Id = P.Product_Id INNER JOIN Payments Py ON O.Order_Id = Py.Order_Id ;

 -- Filter and Joins [Specific Order Details]
 -- Show the order ID, customer's full name, and product's category for all orders that had a payment type of 'CARD'.
 SELECT o.Order_Id, c.Full_Name, p.Category FROM Orders o INNER JOIN Customers c ON o.Customer_Id = c.Customer_Id
INNER JOIN Products P ON o.Product_Id = p.Product_Id WHERE o.Payment_Type = 'CARD' ; 
-- Above query is show data using inner join with condition paymenttype card. 
